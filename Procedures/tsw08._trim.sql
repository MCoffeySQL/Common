...
BEGIN
  ...
  BEGIN  
    DELETE FROM TSWData08.dbo.ctypeAvailable WHERE cTypeAvailableID IN (SELECT TOP (@rowsToDelete) cTypeAvailableID AS id FROM TSWData08.dbo.cTypeAvailable WHERE beginDate < DATEADD(DAY,@daysToKeep,GETDATE()));
  END

  SET @message = 'Trimming - TSWData08.dbo.Intf_MessageProc';
  RAISERROR(@message,0,1)WITH NOWAIT;
  WHILE EXISTS(SELECT 1 FROM TSWData08.dbo.Intf_MessageProc WHERE processTime < DATEADD(DAY,-(365 * 3),GETDATE()))
  BEGIN  
    DELETE TOP (10000) 
    FROM TSWData08.dbo.Intf_MessageProc 
    WHERE processTime < DATEADD(DAY,-(365 * 3),GETDATE())
  END
  ...
  BEGIN    
    DELETE FROM TSWData08.dbo.t_ARLineItem WHERE arLineItemID IN (SELECT TOP (@rowsToDelete) arLineItemID AS id FROM TSWData08.dbo.t_ARLineItem WHERE [transactionDate] < DATEADD(DAY,@daysToKeep,GETDATE()));
  END
  ...
  BEGIN    
    DELETE FROM TSWData08.dbo.cTypeAvailableNew WHERE cTypeAvailableID IN (SELECT TOP (@rowsToDelete) cTypeAvailableID AS id FROM TSWData08.dbo.cTypeAvailableNew WHERE [endDate] < DATEADD(DAY,@daysToKeep,GETDATE()));
  END
  ...
  BEGIN    
    DELETE FROM TSWData08.dbo.cUsageDetail WHERE usageDetailID IN (SELECT TOP (@rowsToDelete) usageDetailID AS id FROM TSWData08.dbo.cUsageDetail WHERE [auditDate]  < DATEADD(DAY,@daysToKeep,GETDATE()));
  END
  ...
  BEGIN    
    DELETE FROM TSWData08.dbo.roomEvents WHERE roomEventsHistoryID IN (SELECT TOP (@rowsToDelete) roomEventsHistoryID AS id FROM TSWData08.dbo.cUsageDetail WHERE [Date]  < DATEADD(DAY,@daysToKeep,GETDATE()));
  END
END
