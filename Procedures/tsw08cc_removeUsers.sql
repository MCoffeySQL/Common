...
DROP PROCEDURE IF EXISTS dba.tsw08cc_removeUsers;
GO

CREATE OR ALTER PROCEDURE dba.tsw08cc_removeUsers
AS
BEGIN
  SET NOCOUNT ON;
  ...
  SELECT
    [name],
    0
  FROM tswdata08_clientCustom.sys.database_principals
  ...
  BEGIN
    SELECT TOP 1
      @id = id,
      @sqlStr = 'USE tswdata08_clientCustom; DROP USER [' + userName + '];'
    FROM #thisTable
    WHERE processed = 0
